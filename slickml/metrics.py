...
# --------------------------------------------------------------------------------------
# Loading Libraries
from sklearn.metrics import auc, f1_score, precision_recall_curve, accuracy_score,\
...
class BinaryClassificationMetrics:
    ...
    Parameters:
               ...
               - y_pred_proba: list[float] 
                   List of predicted probability for the positive class (class=1 or y_pred_proba[:, 1] in scikit-learn)
               - threshold: float, optional (default=0.5)
                   Threshold value for mapping y_pred_prob to y_pred (> is used not >=)
               - average_method: string, optional (default="binary")
                   Method to calculate the average of the metric. Possible values are "micro", "macro", "weighted", "binary"
               - precision_digits: int, optional (default=3)
                   The number of precision digits to display scores in final dataframe
               - figsize: tuple, optional (default=(12,12))
                   Figure size for scores vs thresholds plots
               - display_df: boolean, optional (default=True)
                   Flag to display metrics DataFrame with CSS formatting
                   
    """

    def __init__(self, y_true, y_pred_proba, threshold=0.5, average_method="binary", precision_digits=3, figsize=(12, 12), display_df=True):
        """
        Default constructor 
        """
        ...
        self.precision_list, self.recall_list, self.pr_thresholds = self._precision_recall_curve()
        ...
        self.sens_spec_index, self.sens_spec_threshold = self._threshold_sens_spec()
        self.prec_rec_index, self.prec_rec_threshold = self._threshold_prec_rec()
    ...
    def _balanced_accuracy(self):
        ...
        """        
        balanced_accuracy = balanced_accuracy_score(y_true=self.y_true, y_pred=self.y_pred, adjusted=False)
    ...
    def _roc_curve(self):
        ...
        """
        fpr_list, tpr_list, roc_thresholds = roc_curve(y_true=self.y_true, y_score=self.y_pred_proba)
    ...
    def _auc_roc(self):
        ...
        if self.average_method == "binary":
            self.average_method = None
        auc_roc = roc_auc_score(y_true=self.y_true, y_score=self.y_pred_proba, average=self.average_method)
        
        return auc_roc
    
    def _precision_recall_curve(self):
        """
        Function to calculate the precision recall curve elements: precision_list, recall_list, pr_thresholds
        """
        precision_list, recall_list, pr_thresholds = precision_recall_curve(y_true=self.y_true, probas_pred=self.y_pred_proba)
    ...
    def _precision_recall_f1(self):
        ...
        """
        precision, recall, f1, _ = precision_recall_fscore_support(y_true=self.y_true, y_pred=self.y_pred, beta=1.0, average=self.average_method)

        # updating precision, recall, and f1 for binary average method
        if self.average_method == None:
    ...
    def _f2_f50(self):
        ...
        """
        f2 = fbeta_score(y_true=self.y_true, y_pred=self.y_pred, beta=2.0, average=self.average_method)
        f05 = fbeta_score(y_true=self.y_true, y_pred=self.y_pred, beta=0.5, average=self.average_method)
    ...
    def _average_precision(self):
        ...
        """
        average_precision = average_precision_score(y_true=self.y_true, y_score=self.y_pred_proba, average=self.average_method)
    ...
    def _threat_score(self):
        ...
        if self.average_method == "weighted":
            ...
            wp = self.tp/w
            wn = self.tn/w
            threat_score = wp * (self.tp/(self.tp + self.fp + self.fn)) + wn * (self.tn/(self.tn + self.fn + self.fp))
        
        elif self.average_method == "macro":
            threat_score = 0.5 * (self.tp/(self.tp + self.fp + self.fn)) + 0.5 * (self.tn/(self.tn + self.fn + self.fp))
    ...
    def _metrics_dict(self):
        """
        Function to create a dictionary of all calculated metrics based on the precision digits and average method
        """
        metrics_dict = {"Accuracy" : round(self.accuracy, self.precision_digits),
    ...
    def _metrics_df(self):
        """
        Function to create a pandas DataFrame of all calculated metrics based on the precision digits and average method
        """
        # update None average_method back to binary for printing
        if self.average_method == None:
            self.average_method = "binary"
            
        metrics_df = pd.DataFrame(data=self.metrics_dict, index=[F"""Threshold = {self.threshold:.3f} | Average = {self.average_method.title()}"""])
        metrics_df = metrics_df.reindex(columns = ["Accuracy", "Balanced Accuracy", "ROC AUC", "PR AUC", "Precision", "Recall",\
        ...
        if self.display_df:
            display(metrics_df.style.background_gradient(cmap=cm)\
    ...
    def _threshold_sens_spec(self):
        """
        Function to calculate the threshold that maximizes sensitivity-specificity curve
        """
    ...
    def _threshold_prec_rec(self):
        """
        Function to calculate the threshold that maximizes precision-recall curve
        """
    ...
    def _thresholds_dict(self):
        ...
        """
        thresholds_dict = {"Youden" : self.youden_threshold,
    ...
    def _plot_thresholds(self):
        ...
        thr_set1 = np.arange(min(self.roc_thresholds), max(self.roc_thresholds), 0.01)
        thr_set2 = np.arange(min(self.pr_thresholds), max(self.pr_thresholds), 0.01)
        f1_list = [2 * (self.precision_list[i] * self.recall_list[i]) / (self.precision_list[i] + self.recall_list[i]) for i in range(len(self.precision_list))]
        queue_rate_list = [(self.y_pred_proba >= thr).mean() for thr in thr_set2]
        accuracy_list = [accuracy_score(self.y_true, (self.y_pred_proba >= thr).astype(int)) for thr in thr_set1]
        ...
        plt.subplot(2,2,1)
        plt.plot(self.fpr_list, self.tpr_list, color="red", label=F"AUC = {self.auc_roc:.3f}")
        plt.plot(self.fpr_list[self.youden_index], self.tpr_list[self.youden_index], marker="o", color="navy", ms =10 )
        plt.xlim([-0.01, 1.01])
        plt.ylim([-0.01, 1.01])
        ...
        plt.annotate(F"Threshold = {self.youden_threshold:.3f}",
                     xy=(self.fpr_list[self.youden_index], self.tpr_list[self.youden_index]), xycoords="data",
                     xytext=(self.fpr_list[self.youden_index]+0.4, self.tpr_list[self.youden_index]-0.4),
        ...
        if self.sens_spec_threshold <= 0.5:
            plt.annotate(F"Threshold = {self.sens_spec_threshold:.3f}",
                         xy=(self.sens_spec_threshold, 0.05), xycoords="data",
        ...
        plt.subplot(2,2,3)
        plt.plot(self.recall_list, self.precision_list, color="red", label=F"PR AUC ={self.auc_pr:.3f}")
        plt.plot(self.recall_list[self.prec_rec_index], self.precision_list[self.prec_rec_index], marker="o", color="navy", ms=10)
        plt.axvline(x=self.recall_list[self.prec_rec_index], ymin=self.recall_list[self.prec_rec_index], ymax=self.precision_list[self.prec_rec_index], color="navy", ls="--")
        plt.xlim([-0.01, 1.01])
        plt.ylim([-0.01, 1.01])
        ...
        plt.annotate(F"Threshold = {self.prec_rec_threshold:.3f}",
                     xy=(self.recall_list[self.prec_rec_index], self.precision_list[self.prec_rec_index]), xycoords="data",
                     xytext=(self.recall_list[self.prec_rec_index]-0.4, self.precision_list[self.prec_rec_index]-0.4),
                     arrowprops=dict(facecolor="black", shrink=0.05),
                     horizontalalignment="left",
                     verticalalignment="bottom")
        ...
        plt.subplot(2,2,4)
        plt.plot(self.pr_thresholds, self.precision_list[1:], label="Precision")
        ...
        if self.prec_rec_threshold <= 0.5:
            plt.annotate(F"Threshold = {self.prec_rec_threshold:.3f}",
                         xy=(self.prec_rec_threshold, 0.03), xycoords="data",
